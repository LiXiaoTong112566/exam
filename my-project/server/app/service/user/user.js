 ...
 * @Last Modified time: 2019-04-22 20:08:36
 * @Func 用户及其权限控制的业务逻辑
 */
...
class UserService extends Service {
    ...
    async show(param) {
        if (param === 'user') { // 读取用户数据
            let sql = `select user_name,user_pwd,user_id,avatar,identity.identity_text as identity_text from user,identity where user.identity_id=identity.identity_id`;
            let result = await this.app.mysql.query(sql);
            let sqlUser = `select user_name,user_pwd,user_id,avatar,identity_id as identity_text from user where identity_id is null`;
